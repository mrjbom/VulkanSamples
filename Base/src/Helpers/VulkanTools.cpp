...
void vulkanTools::setImageLayout(
...
void vulkanTools::insertImageMemoryBarrier(
    VkCommandBuffer cmdbuffer,
    VkImage image,
...
void vulkanTools::exitFatal(const std::string& message, int32_t exitCode)
{
    if (!errorModeSilent) {
...
void vulkanTools::exitFatal(const std::string& message, VkResult resultCode)
{
...
VkShaderModule vulkanTools::loadShader(VkDevice device, const char* fileName)
{
    ...
    if (!is.is_open()) {
        throw MakeErrorInfo((std::string)"Could not open shader file \"" + fileName + "\"");
    }
...
bool vulkanTools::fileExists(const std::string& filename)
{
    std::ifstream f(filename.c_str());
    return !f.fail();
}

uint32_t vulkanTools::alignedSize(uint32_t value, uint32_t alignment)
{
